...
# создадим граф c весовой функцией
...
def solve(n, m, edges):
    ...
    for i in range(m):
        ...
        if not jopa(a, b):
            union()
            answer += c
            answer_edges.append([country[a], country[b], c])

    print('Вес минимального остовного дерева:', answer)

    # Посмотрим на остовное дерево
    G = nx.Graph()
    G.add_weighted_edges_from(answer_edges)
    weights = nx.get_edge_attributes(G, 'weight')
    plt.figure(figsize=(25, 25))
    pos = nx.planar_layout(G)
